...
//  SuperMap_iOS_Controller
//
//  Created by CNSLab-mini on 2016/9/1.
//  Copyright © 2016年 bochengw.twbbs.org. All rights reserved.
//
...
class ViewController: UIViewController, UITableViewDelegate, UITableViewDataSource, UISearchBarDelegate, GMSMapViewDelegate {
    ...
    let socket = SocketIOClient(socketURL: NSURL(string: "http://140.118.155.49:4000")!, options: [.Log(true), .ForcePolling(true)])
    ...
    @IBAction func autoJump(sender: AnyObject) {
        if !autoJump {
            ...
            timer = NSTimer.scheduledTimerWithTimeInterval(1.0, target: self, selector: #selector(ViewController.jump(_:)), userInfo: nil, repeats: true)
        } else {
            timer.invalidate()
            ...
            googleDataProvider.parseJsonDataDirection(self.currentState.coordinate, to: destCoordinate, completion: { (stepsArray, overviewObject, error) -> Void in
                ...
                if error != nil {print(error)}
                    ...
                    for step in self.steps {
                        ...
                        for var i: UInt = 0; i < in_path!.count(); i += 1 { self.coordinateList.append(in_path!.coordinateAtIndex(i)) }
